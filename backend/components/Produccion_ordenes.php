...
class Produccion_ordenes extends Component
{
    ...
    public function getOrdenes($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $asiento= Ordenesprod::find()->where(["isDeleted"=>0])->all();
        }else{
            $asiento= Ordenesprod::find()->where(["isDeleted"=>0])->one();
        }
    }
